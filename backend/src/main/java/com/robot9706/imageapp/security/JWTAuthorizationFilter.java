...
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request) {
        ...
        if (token != null) {
            ...
            if (claims.containsKey(ROLES_CONST)) {
                authorities = claims.get(ROLES_CONST).asList(String.class).stream().map(SimpleGrantedAuthority::new).collect(Collectors.toList());
            } else {
